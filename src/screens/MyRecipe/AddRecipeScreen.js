...
const AddRecipeScreen = ({ navigation }) => {
  ...
  const addIngredientFromInput = () => {
    if (newIngredientText.trim()) {
      ...
      const newIngredient = {
        name: name,
        count: amount // 서버 Recipe 객체는 'count'를 사용
      };
  ...
  const saveRecipe = async () => {
    ...
    if (recipeSteps.length === 0) {
      Alert.alert('알림', '레시피를 최소 1단계 이상 입력해주세요.');
      return;
    }
    ...
    try {
      ...
      // ## 서버가 요구하는 Recipe 객체 형태로 데이터를 구성 ##
      const newRecipeData = {
        name: recipeName,
        time: cookingTime,
        ...
        recipe: recipeSteps,
        tools: 0, // 기본값 0, 필요시 이 화면에서 도구 선택 기능 추가 필요
        type: 0,  // 기본값 0
      };

      await axios.post(`${API_BASE_URL}/recipes/added`, newRecipeData, {
