...
const EditRecipeScreen = ({ navigation, route }) => {
  ...
  const [cookingTime, setCookingTime] = useState(recipe.time);
  const [ingredients, setIngredients] = useState([
      ...(recipe.mainIngredients || []), 
      ...(recipe.subIngredients || [])
  ]);
  const [recipeSteps, setRecipeSteps] = useState(recipe.recipe || []);
  const [newIngredientText, setNewIngredientText] = useState('');
  const [newRecipeStep, setNewRecipeStep] = useState('');
  const [isSaving, setIsSaving] = useState(false);

  // --- ## 기능 추가를 위한 상태 ## ---
  const [tools, setTools] = useState(recipe.tools || 0);
  const [type, setType] = useState(recipe.type || 0);
  ...
  const saveRecipe = async () => {
    ...
    try {
      ...
      const updatedRecipeData = {
        ...
        recipe: recipeSteps,
        tools: tools, // 수정된 도구 값
        type: type,   // 수정된 종류 값
      };
      
      await axios.put(`${API_BASE_URL}/api/recipes/added`, updatedRecipeData, {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.saveButton} onPress={saveRecipe} disabled={isSaving}>
          {isSaving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveButtonText}>수정 완료</Text>}
        </TouchableOpacity>
      </View>
      <SelectionModal 
...
const SelectionModal = ({ isVisible, onClose, title, data, isMultiSelect, onConfirm, currentSelection }) => {
    ...
    // 모달이 열릴 때마다 현재 선택된 값을 state에 반영
    React.useEffect(() => {
